<template>
  <q-page padding>
    <div class="container">
      ...
      <div v-if="newTabActive"><q-input v-model="tabName" label="Название" @keydown.enter="tabAction('add')"/></div>
      <q-tabs
        v-model="tab"
        dense
        class="text-grey"
      ...
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel :name="`tab${tab_index}`" v-for="(tab,tab_index) in tabs" :key="tab_index">
          ...
          <div v-if="newRecordActive">
            <q-input v-model="new_record.name" label="Название записи" />
            <div class="row q-col-gutter-md" v-for="(param,index) in params" :key="param.id">
              <div class="col-6">
                <p class="no-margin">{{param.name}}</p>
              </div>
              <div class="col-6">

                <q-input v-model="new_record.params[index][param['name']]" label="Название записи" />
              </div>
            </div>
